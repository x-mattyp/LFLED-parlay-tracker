...
export default async function AdminPage({ searchParams }) {
  ...
  return (
    <>
      ...
      <h2>Picks</h2>
      {legacy ? (
        <form action={saveLegacyPicks} className="panel">
          <input type="hidden" name="week_id" value={week.id} />
          <p className="muted small">
            Week {n} was before matchup picking, so type in what each person bet and how it did. Leave a pick blank to remove it.
          </p>
              ...
              return (
                <li key={m.id}>
                  ...
                  <div className="legacyrow">
                    ...
                    <select name={`result_${m.id}`} defaultValue={p?.result || 'pending'} aria-label={`${m.name}'s result`}>
          ...
          <button>Save week {n} picks</button>
        </form>
      ) : (
        <div className="panel">
          <div className="inline">
            <form action={toggleLock}>
              <input type="hidden" name="week_id" value={week.id} />
              <button className="ghost">{week.locked ? 'Unlock picks' : 'Lock all picks now'}</button>
            </form>
            <span className="muted small">
              {week.locked ? 'Nobody can change their pick.' : 'Each game locks on its own at kickoff.'}
            </span>
          </div>
          <p className="muted small">Results grade themselves when games go final. Tap a button to override one.</p>
          {picks.length ? (
            <ul className="gradelist">
              {picks.map((p) => (
                <li key={p.member_id}>
                  <span>
                    <b>{nameOf[p.member_id]}</b>{' '}
                    <span className="muted">{p.team_name ? `${p.team_name} ML` : p.bet}{p.odds ? ` (${p.odds})` : ''}</span>
                  </span>
      ...
      {n === settings.current_week && (
        <>
          ...
          <form action={advanceWeek} className="panel">
            <p className="muted small">The app moves to the next NFL week on its own each morning. Use this only to jump ahead early.</p>
