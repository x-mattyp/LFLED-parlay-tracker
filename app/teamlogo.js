...
export default function TeamLogo({ member, size = 36, className = 'teamlogo' }) {
  ...
  if ((!member.logo_url && !member.team_logo) || broken) {
    return (
  ...
  // Prefer the copy saved in storage; otherwise fetch ESPN's via the app.
  const src = member.logo_url || `/api/logo/${member.id}?v=${encodeURIComponent(member.team_logo.slice(-32))}`;
  return (
    <img
      className={className}
      src={src}
