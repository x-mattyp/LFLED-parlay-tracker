...
async function sleeperTeams(leagueId) {
  ...
  return rosters.map((r) => {
    ...
    return {
      ...
      abbrev: null,
      logo: null,
      owners: u ? [u.display_name].filter(Boolean) : [],
    };
  });
}
...
async function espnTeams(season, leagueId) {
  ...
  const personName = (m) => [m?.firstName, m?.lastName].filter(Boolean).join(' ') || m?.displayName || '';
  return (data.teams || []).map((t) => ({
    id: String(t.id),
    name: t.name || [t.location, t.nickname].filter(Boolean).join(' ') || t.abbrev || `Team ${t.id}`,
    abbrev: t.abbrev || null,
    logo: t.logo || null,
    owners: (t.owners || []).map((o) => personName(memberById[o?.id ?? o])).filter(Boolean),
  }));
}
